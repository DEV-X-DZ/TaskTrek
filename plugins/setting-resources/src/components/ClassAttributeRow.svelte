...
<button class="task-trekTableAttr-content__row" class:hovered class:selected on:contextmenu on:click>
  <button class="task-trekTableAttr-content__row-dragMenu" on:click|stopPropagation={clickMore}>
    <IconMoreV2 size={'small'} />
  </button>
  {#if attribute.isCustom}
    <div class="task-trekChip-item font-medium-12">
      <Label label={settings.string.Custom} />
    </div>
  {/if}
  {#if attribute.icon !== undefined}
    <div class="task-trekTableAttr-content__row-icon">
      <Icon icon={attribute.icon} size={'small'} />
    </div>
  {/if}
  <div class="task-trekTableAttr-content__row-label font-regular-14 grow" class:accent={!attribute.hidden}>
    <Label label={attribute.label} />
  </div>
  {#if attributeMapper}
    <svelte:component this={attributeMapper.component} {...attributeMapper.props} {attribute} />
  {/if}
  <div class="task-trekTableAttr-content__row-type font-medium-12">
  ...
  <div class="task-trekTableAttr-content__row-arrow">
