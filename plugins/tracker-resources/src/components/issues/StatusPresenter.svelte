...
{#if value}
  {#if kind === 'table-attrs'}
    <button class="task-trekTableAttr-content__row-icon-wrapper" on:click>
      <IssueStatusIcon
        {value}
        {size}
        {space}
    ...
    <span
      class="task-trekTableAttr-content__row-label font-medium-12 uppercase grow overflow-label"
