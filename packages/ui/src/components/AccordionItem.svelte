...
<div class="task-trekAccordionItem-container {kind}" class:nested>
  <button
    class="task-trekAccordionItem-header {kind} {size}"
    class:bottomSpace
    class:nested
    class:isOpen
    class:selected
  ...
  >
    <div
      class="task-trekAccordionItem-header__label-wrapper {size === 'large' ? 'heading-medium-16' : 'font-medium-12'}"
      class:withIcon={size === 'medium' && icon !== undefined}
    >
      {#if size === 'large'}
        <div class="task-trekAccordionItem-header__chevron">
      ...
      {/if}
      <span class="task-trekAccordionItem-header__label">
        {#if label}<Label {label} />{/if}
        {#if title}{title}{/if}
        <slot name="title" />
      </span>
      {#if counter !== false}
        <span class="task-trekAccordionItem-header__separator">•</span>
        <span class="task-trekAccordionItem-header__counter">
      ...
      {#if duration !== false}
        <span class="task-trekAccordionItem-header__separator">•</span>
        <span class="task-trekAccordionItem-header__duration">
          {#if typeof duration === 'number'}{duration}{/if}
          <slot name="duration" />
        </span>
      {/if}
    </div>
    <div class="task-trekAccordionItem-header__tools">
      <slot name="actions" />
    </div>
  </button>
  <div class="task-trekAccordionItem-content">
    <slot />
  </div>
</div>
