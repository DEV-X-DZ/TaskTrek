...
<div class="task-trekPopup-container" use:resizeObserver={() => dispatch('changeContent')}>
  <Scroller padding={'var(--spacing-0_5)'} gap={'flex-gap-0-5'}>
    {#each items as item, i}
      <!-- svelte-ignore a11y-mouse-events-have-key-events -->
      <button
        class="task-trekPopup-row"
        on:mouseover={(ev) => {
      ...
      >
        {#if withIcons}
          <div class="task-trekPopup-row__icon">
            {#if item.icon}<Icon icon={item.icon} size={'small'} />{/if}
          </div>
        {/if}
        {#if item.description}
          <div class="task-trekPopup-row__labels-wrapper">
            <div class="task-trekPopup-row__label overflow-label">
              <Label label={item.label} params={item.params ?? params} />
            </div>
            <div class="task-trekPopup-row__label small dark">
              <Label label={item.description} params={item.paramsDescription ?? params} />
            </div>
          </div>
        {:else}
          <div class="task-trekPopup-row__label"><Label label={item.label} params={item.params ?? params} /></div>
        {/if}
        <div class="task-trekPopup-row__icon">
